...
import akka.actor.{Actor, ActorLogging, ActorSystem, Cancellable, Props, Scheduler, Timers}
...
//  }
  ...
  class MyTimerActor extends Actor with ActorLogging {
    override def receive: Receive = {
      case message =>
        log.info(message.toString)
        val cancellable = system.scheduler.scheduleOnce(1.second) {
          context.stop(self)
        }
        context.become(scheduleTask(cancellable))
    }

    def scheduleTask(cancellable: Cancellable): Receive = {
      case message =>
        log.info(message.toString)
        cancellable.cancel()
        val newCancellable = system.scheduler.scheduleOnce(1.second) {
          context.stop(self)
        }
        context.become(scheduleTask(newCancellable))
    }
  }

//  val myTimerActor = system.actorOf(Props[MyTimerActor], "timer")
//  myTimerActor ! "msg1"
//  Thread.sleep(500)
//  myTimerActor ! "msg2"
//  Thread.sleep(1500)
